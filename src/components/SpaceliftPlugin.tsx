...
export const SpaceliftPlugin: React.FC = () => {
    ...
    useEffect(() => {
        const initPlugin = async () => {
            try {
                ...
                // Get Spacelift token from Auth Lambda via Cortex proxy
                const lambdaUrl = process.env.SPACELIFT_AUTH_LAMBDA_URL || 'https://your-api-gateway-id.execute-api.us-east-1.amazonaws.com/prod/auth';
                
                const response = await CortexApi.proxyFetch(lambdaUrl, {
                    method: 'POST',
                    headers: {
    ...
    const getRunStateClass = (state: string): string => {
    ...
    const getStackHealthClass = (metrics: StackMetrics): string => {
        const { totalRuns, successfulRuns, lastRunState } = metrics;
        
        if (totalRuns === 0) return '';
        
        const successRate = (successfulRuns / totalRuns) * 100;
        
        if (lastRunState === 'FAILED') {
            return 'stack-health-error';
        }
        
        if (successRate >= 95) return 'stack-health-excellent';
        if (successRate >= 80) return 'stack-health-good';
        if (successRate >= 60) return 'stack-health-warning';
        return 'stack-health-error';
    };
                            ...
                            <div className="stack-card">
                                ...
                                <div className="stack-header">
                                    <div>
                                        ...
                                        {metrics.totalRuns > 0 && (
                                            <div className={`stack-health ${getStackHealthClass(metrics)}`}>
                                                Health: {Math.round((metrics.successfulRuns / metrics.totalRuns) * 100)}%
                                            </div>
                                        )}
                                    </div>
                                    <Badge 
                                ...
                                <div className="stack-metrics">
                                    ...
                                    <div className="stack-metric">
                                        {metrics.totalRuns > 0 ? (
                                            <span className={`metric-number ${getSuccessRateClass((metrics.successfulRuns / metrics.totalRuns) * 100)}`}>
